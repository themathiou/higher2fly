...
export default defineConfig({
  ...
  theme: {
    ...
    animation: {
      keyframes: {
        'fadein-down': '{from {opacity: 0.1;transform: translateY(-20px);}to {opacity: 1;transform: translateY(0);}}',
        'fadein-left': '{from {opacity: 0.1;transform: translateX(20px);}to {opacity: 1;transform: translateX(0);}}'
      }
    }
  },
  shortcuts: [
  ...
  safelist: [...socials.map(social => `i-mdi-${social.name}`), 'i-mdi-content-copy', 'i-mdi-check']
});
